...
{
    int idx_g, idx_l, idz_g, idz_l, id_l, i;
    int n_idx, n_idz, n_id_all;
    float *id_layer = NULL, *id_layer_grid_all = NULL;
    int i_idx = 0, i_idz = 0;
    struct grid_id_in_layer *idx, *idz, *id_layer_grid;

    idx = (struct grid_id_in_layer*)malloc(MAX_LAYER_POINTS*sizeof(struct grid_id_in_layer));
    idz = (struct grid_id_in_layer*)malloc(MAX_LAYER_POINTS*sizeof(struct grid_id_in_layer));
    id_layer_grid = (struct grid_id_in_layer*)malloc(MAX_LAYER_POINTS*sizeof(struct grid_id_in_layer));
    ...
    for (idx_g = 0; idx_g < nx; idx_g++) {
        for (idx_l = 0; idx_l < npoints_interfaces-1; idx_l++) {
            if (   (xvec[idx_g]-layer_x[idx_l])*(xvec[idx_g]-layer_x[idx_l+1]) <= 0
                ...
                idx[i_idx].id = idx_l + (xvec[idx_g]-layer_x[idx_l])/(layer_x[idx_l+1]-layer_x[idx_l]);
                idx[i_idx].ifxvec = 1;
                idx[i_idx].xvec   = xvec[idx_g];
                i_idx++;
            }
        }
    }
    ...
    for (idz_g = 0; idz_g < nz; idz_g++) {
        for (idz_l = 0; idz_l < npoints_interfaces-1; idz_l++) {
            if (   (zvec[idz_g]-layer_z[idz_l])*(zvec[idz_g]-layer_z[idz_l+1]) <= 0
                ...
                idz[i_idz].id = idz_l + (zvec[idz_g]-layer_z[idz_l])/(layer_z[idz_l+1]-layer_z[idz_l]);
                idz[i_idz].ifzvec = 1;
                idz[i_idz].zvec   = zvec[idz_g];
                i_idz++;
            }
        }
    }
    ...
    merge(idx, idz, n_idx, n_idz, id_layer_grid, &n_id_all);
    ...
    for (i = 0; i < n_id_all; i++)
        id_layer_grid_all[i] = id_layer_grid[i].id;
    ...
    /* Re-assign the xvec, zvec value to avoid the precision problem */
    for (i = 0; i < n_id_all; i++) {
        if ( id_layer_grid[i].ifxvec == 1 )
            layer_of_grid_x[i] = id_layer_grid[i].xvec;
        if ( id_layer_grid[i].ifzvec == 1 )
            layer_of_grid_z[i] = id_layer_grid[i].zvec;
    }
                  ...
                  int *ix, int *iz, int *loc_type, float *area1, float *area2, float *theta)
{
    ...
    for (i_point = 0; i_point < npoints_layer-1; i_point++) {
        ...
        ix1 = floor( (layer_of_grid_x[i_point]  -xvec[0]) / dx);
        iz1 = floor( (layer_of_grid_z[i_point]  -zvec[0]) / dz);

        ix2 = floor( (layer_of_grid_x[i_point+1]-xvec[0]) / dx);
        iz2 = floor( (layer_of_grid_z[i_point+1]-zvec[0]) / dz);
        ...
        if ( fabs(layer_of_grid_x[i_point  ]-xvec[ix_para]) <= 1e-6*dx && ( iz1 == iz_para || iz1 == iz_para+1 ) &&
             fabs(layer_of_grid_z[i_point+1]-zvec[iz_para]) <= 1e-6*dz && ( ix2 == ix_para || ix2 == ix_para+1 ) ){
            area1_para = ( layer_of_grid_x[i_point+1] - xvec[ix_para] ) *
                         ( layer_of_grid_z[i_point  ] - zvec[iz_para] )/2;
            area2_para = dx*dz - area1_para;
            loc_type_para = 1;
        }
        else if ( fabs(layer_of_grid_z[i_point  ]-zvec[iz_para  ]) <= 1e-6*dz && (ix1 == ix_para || ix1 == ix_para+1) &&
                  fabs(layer_of_grid_x[i_point+1]-xvec[ix_para+1]) <= 1e-6*dx && (iz2 == iz_para || iz2 == iz_para+1) ) {
            area1_para = (-layer_of_grid_x[i_point  ] + xvec[ix_para+1] ) *
                         ( layer_of_grid_z[i_point+1] - zvec[iz_para  ] ) /2;
            area2_para = dx*dz - area1_para;
            loc_type_para = 4;
        }
        else if ( fabs(layer_of_grid_x[i_point  ]-xvec[ix_para  ]) <= 1e-6*dx && ( iz1 == iz_para || iz1 == iz_para+1 ) &&
                  fabs(layer_of_grid_x[i_point+1]-xvec[ix_para+1]) <= 1e-6*dx && ( iz2 == iz_para || iz2 == iz_para+1 ) ) {
            ...
            loc_type_para = 2;
                  ...
                  fabs(layer_of_grid_z[i_point+1]-zvec[iz_para+1]) <= 1e-6*dz && (ix2 == ix_para ) ) {
            area2_para = ( layer_of_grid_x[i_point+1] - xvec[ix_para  ] ) *
                         (-layer_of_grid_z[i_point  ] + zvec[iz_para+1] )/2;
            area1_para = dx*dz - area2_para;
            loc_type_para = 3;
        }
        else if ( fabs(layer_of_grid_x[i_point  ]-xvec[ix_para+1]) <= 1e-6*dx && (iz1 == iz_para || iz1 == iz_para+1) &&
                  fabs(layer_of_grid_z[i_point+1]-zvec[iz_para+1]) <= 1e-6*dz && (ix2 == ix_para || ix2 == ix_para+1) ) {
            area1_para = ( xvec[ix_para+1] - layer_of_grid_x[i_point+1] ) *
                         ( zvec[iz_para+1] - layer_of_grid_z[i_point  ] ) / 2;
            area2_para = dx * dz - area1_para;
            loc_type_para = 6;
        }
        else if ( fabs(layer_of_grid_z[i_point  ]-zvec[iz_para  ]) <= 1e-6*dz && (ix1 == ix_para )  &&
                  fabs(layer_of_grid_z[i_point+1]-zvec[iz_para+1]) <= 1e-6*dz && (ix2 == ix_para ) ) {
        ...
        else if ( fabs(layer_of_grid_x[i_point+1]-xvec[ix_para]) <= 1e-6*dx && (iz2 == iz_para ) &&
                  fabs(layer_of_grid_z[i_point  ]-zvec[iz_para]) <= 1e-6*dz && (ix1 == ix_para ) ) {
            ...
            loc_type_para = 7;
                  ...
                  fabs(layer_of_grid_x[i_point  ]-xvec[ix_para+1]) <= 1e-6*dx && (iz1 == iz_para ) ) {
            area2_para = ( ( layer_of_grid_z[i_point+1] - zvec[iz_para] )
                          +( layer_of_grid_z[i_point  ] - zvec[iz_para] ) )* dx/2;
            area1_para = dx*dz - area2_para;
            loc_type_para = 8;
        }
        else if ( fabs(layer_of_grid_x[i_point+1]-xvec[ix_para  ]) <= 1e-6*dx && (iz2 == iz_para || iz2 == iz_para+1) &&
                  fabs(layer_of_grid_z[i_point  ]-zvec[iz_para+1]) <= 1e-6*dz && (ix1 == ix_para || ix1 == ix_para+1) ) {
            area1_para = ( layer_of_grid_x[i_point  ] - xvec[ix_para  ] ) *
                         (-layer_of_grid_z[i_point+1] + zvec[iz_para+1] )/2;
            area2_para = dx*dz - area1_para;
            loc_type_para = 9;
        }
        else if ( fabs(layer_of_grid_z[i_point+1]-zvec[iz_para  ]) <= 1e-6*dz && (ix2 == ix_para || ix2 == ix_para+1) &&
                  fabs(layer_of_grid_x[i_point  ]-xvec[ix_para+1]) <= 1e-6*dx && (iz1 == iz_para                        ) ) {
        ...
        else if ( fabs(layer_of_grid_x[i_point+1]-xvec[ix_para+1]) <= 1e-6*dx && (iz2 == iz_para || iz2 == iz_para+1) &&
                  fabs(layer_of_grid_z[i_point  ]-zvec[iz_para+1]) <= 1e-6*dz && (ix1 == ix_para || ix1 == ix_para+1) ) {
            area2_para = ( xvec[ix_para+1] - layer_of_grid_x[i_point  ] ) *
            ...
            if (iz_para > 0 && iz_para < nz-1 && ix_para > 0 && ix_para < nx-1 ) {
                if ( fabs(layer_of_grid_x[i_point]-layer_of_grid_x[i_point+1])>1e-6 &&
                     fabs(layer_of_grid_z[i_point]-layer_of_grid_z[i_point+1])>1e-6)
...
{
    int i,j,Increment;
    struct grid_id_in_layer tmp;

    for (Increment = N/2; Increment > 0; Increment /= 2) {
        for (i = Increment; i < N; i++) {
            tmp = arr[i];
            for (j = i; j >= Increment; j -= Increment) {
                if(tmp.id < arr[j - Increment].id)
           ...
           int n1, int n2, struct grid_id_in_layer *arr3, int *n3)
{
    ...
    while (i < n1 && j < n2) {
        ...
        if ( fabs(arr1[i].id - arr2[j].id) <= 2e-4) {
            arr3[k] = arr2[j];
            if (arr1[i].ifxvec == 1){
                arr3[k].ifxvec = 1;
                arr3[k].xvec = arr1[i].xvec;
            }
            if (arr1[i].ifzvec == 1){
                arr3[k].ifzvec = 1;
                arr3[k].zvec = arr1[i].zvec;
            }
            k++;
            j++;
            i++;
